...
test('Chrona integrates buddy and spectator sharing across home and run views', async () => {
  ...
  assert.match(home, /className="weekly-streak-summary"[\s\S]*className="card-ring-counters"[\s\S]*<WeeklyRing set=\{set\} \/>[\s\S]*<FireStrip set=\{set\} compact \/>/)
  assert.match(css, /\.weekly-streak-summary \{[^}]*align-items: center;[^}]*gap: 12px;[^}]*margin-left: auto;/)
