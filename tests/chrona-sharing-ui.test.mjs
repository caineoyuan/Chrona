...
test('Chrona integrates buddy and spectator sharing across home and run views', async () => {
  ...
  assert.match(home, /className="weekly-streak-progress"[\s\S]*\{streakCounter\}[\s\S]*<FireStrip set=\{set\} compact \/>[\s\S]*className="card-ring-counters"[\s\S]*<WeeklyRing set=\{set\} \/>/)
  assert.match(css, /\.weekly-streak-progress \{[^}]*align-items: center;[^}]*gap: 10px;/)
  assert.match(css, /\.weekly-streak > \.card-ring-counters \{[^}]*margin-left: auto;/)
  ...
  assert.match(css, /\.streak-calendar-day\.current \.streak-calendar-marker::after \{[^}]*bottom: -3px;/)
