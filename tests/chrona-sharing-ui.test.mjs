...
test('Chrona integrates buddy and spectator sharing across home and run views', async () => {
  ...
  assert.match(home, /participatingFriend \|\| weekly[\s\S]*participatingFriend && <BuddyRing streak=\{buddyStreak\} \/>[\s\S]*weekly && <WeeklyRing set=\{set\} \/>/)
  assert.match(home, /\(weekly \|\| !participatingFriend\) && <FireStrip set=\{set\} compact=\{weekly\} \/>/)
