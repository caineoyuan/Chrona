...
test('Chrona integrates buddy and spectator sharing across home and run views', async () => {
  ...
  assert.match(home, /className="weekly-streak-history"[\s\S]*className="card-ring-counters"[\s\S]*<WeeklyRing set=\{set\} \/>[\s\S]*<FireStrip set=\{set\} compact \/>/)
  assert.match(css, /\.weekly-streak-history \{[^}]*flex-direction: column;[^}]*margin-left: auto;/)
  ...
  assert.match(run, /className="streak-calendar-marker"[\s\S]*size=\{48\}[\s\S]*className="streak-calendar-day-number"/)
  assert.match(css, /\.streak-calendar-day\.current \.streak-calendar-day-number \{[^}]*border-radius: 50%;/)
  assert.match(css, /\.streak-calendar-marker \{[^}]*width: 48px;[^}]*height: 48px;/)
  ...
  assert.match(run, /name=\{day\.completed \? 'fire-element' : 'snowflake'\} size=\{48\}/)
