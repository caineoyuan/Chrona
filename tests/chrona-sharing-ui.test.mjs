...
test('Chrona integrates buddy and spectator sharing across home and run views', async () => {
  ...
  assert.match(home, /className=\{`card-head[\s\S]*weekly-card-head[\s\S]*sharedPeople\.length[\s\S]*<WeeklyRing set=\{set\} \/>/)
  assert.match(home, /weekly[\s\S]*\? <FireStrip set=\{set\} compact \/>/)
  assert.match(css, /\.weekly-card-head \.card-ring-counters \{[^}]*margin-left: auto;/)
  assert.match(css, /\.weekly-card-head\.has-shared-people \{[^}]*align-items: flex-end;/)
  ...
  assert.match(css, /\.streak-calendar-marker \.streak-calendar-day-number \{[^}]*color: #fff;[^}]*font-weight: 700;/)
