...
test('Medira sharing modal preserves mobile scrolling and accessible controls', async () => {
  ...
  assert.match(source, /const fallbackOwnerProfile = \{[\s\S]*role: 'owner',[\s\S]*canViewHistory: true,/)
  assert.match(source, /if \(!selectedProfile\.ownerUserId\) \{[\s\S]*medicationPermissions\(medication\)\.role === 'owner'/)
  assert.match(source, /if \(sharing\.status !== 'ready'\) return/)
